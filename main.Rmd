...
diab <- diab %>% mutate(
  ...
  Diabetes_binary = factor(x = Diabetes_binary, levels = c(0,1), labels = c("No Diabetes", "Diabetes")),
...
g + geom_bar(aes(fill = diab_subset$Diabetes_binary))

g2 <-ggplot(data = diab_subset, aes(x= diab_subset$Fruits))
g2 + geom_bar(aes(fill= diab_subset$Diabetes_binary))
```
