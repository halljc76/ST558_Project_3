...
```{r}
logreg1Pred <- as.numeric(predict(logreg1, test %>% 
                           select(-Education),
                       type = "prob")[,2] > 0.5)
logreg2Pred <- as.numeric(predict(logreg2, test,
                       type = "prob")[,2] > 0.5)
logreg3Pred <- as.numeric(predict(logreg3, test,
                       type = "prob")[,2] > 0.5)

lr1CM <- confusionMatrix(as.factor(logreg1Pred), 
          as.factor(as.numeric(as.factor(test$Diabetes_binary)) - 1),
          positive = "1")
lr2CM <- confusionMatrix(as.factor(logreg2Pred), 
            as.factor(as.numeric(as.factor(test$Diabetes_binary)) - 1),
          positive = "1")
lr3CM <- confusionMatrix(as.factor(logreg3Pred), 
          as.factor(as.numeric(as.factor(test$Diabetes_binary)) - 1),
          positive = "1")
```
...
getAccSenSpec <- function(c) {
  return(data.frame(c$overall["Accuracy"],
         c$byClass["Sensitivity"],
         c$byClass["Specificity"],
         c$overall["Kappa"]))
}

logregResults <- sapply(list(lr1CM, lr2CM, lr3CM), 
       function(x) {
         getAccSenSpec(x)
       })
row.names(logregResults) <- c("Accuracy", "Sensitivity", "Specificity", 
                              "Kappa")
colnames(logregResults) <- c("LR1", "LR2", "LR3")
# lr1CM
# lr2CM
# lr3CM
logregResults
...
Because the focus of this model is health-based, it is reasonable to conclude that
prioritizing a **lack of false negatives** (i.e., increasing sensitivity) is 
a viable criterion on which to select a best model. Therefore, the candidate model
chosen is that which has the highest sensitivity above (note LR1, LR2, and LR3
refer to the respective first, second, or third logistic regression model). Note that we might also consider which model has the highest Kappa, $\kappa$, which indicates the agreement of the test predictions to the test observations (the value should be closer to $+1$).

```{r}
candidateLRModel <- c(logreg1, logreg2, logreg3)[which.max(logregResults[2,])]
```
...
```{r}
ctResults <- sapply(list(confMatCT), 
       function(x) {
         getAccSenSpec(x)
       })
row.names(ctResults) <- c("Accuracy", "Sensitivity", "Specificity", "Kappa")
colnames(ctResults) <- c("Classification Tree")
ctResults
```
...
```{r}
nbResults <- sapply(list(confMatNBC), 
       function(x) {
         getAccSenSpec(x)
       })
row.names(nbResults) <- c("Accuracy", "Sensitivity", "Specificity", "Kappa")
colnames(nbResults) <- c("Naive Bayes")
nbResults
```
